...
const SellerTab: React.FC<SellerTabProps> = ({ scope, overview, level }) => {
    ...
    <>
      ...
      <div className="py-2">
        <div className="max-w-[1600px] mx-auto px-6 lg:px-12 space-y-1">
          ...
          {/* ── Active Market KPIs ─────────────────────────── */}
          <Divider label="Active Market Snapshot" tier="idx" />
          ...
          <H3HeatmapSection scope={scope} level={level} defaultMetric="dom" />

          {/* ── Seasonal Patterns ─────────────────────────── */}
          {(isMarket || isRegion || isCommunity) && (
            <>
              <Divider label="Seasonal Listing Patterns" tier="idx" />
          ...
          {/* ── Luxury Tiers (market-only) ────────────────── */}
          {isMarket && luxuryTiers && (
            <>
              <Divider label="Luxury Tier Inventory" tier="idx" />
              <LuxuryTierCards tiers={luxuryTiers} />
            </>
          )}
          ...
          {/* ── Top Communities (region-only) ─────────────── */}
          {isRegion && (
            <>
              <Divider label="Top Communities" tier="idx" />
              <div className="bg-white border border-gray-100 p-3">
                <TopCommunitiesGrid data={(scope as RegionScope).getTopCommunities()} />
              </div>
            </>
          )}
