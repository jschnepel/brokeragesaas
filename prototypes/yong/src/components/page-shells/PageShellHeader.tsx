...
import { GaugeMini } from '../../pages/analytics/components/widgets';
...
function rewriteBreadcrumbs(crumbs: Breadcrumb[], basePath: string, pageLabel?: string): Breadcrumb[] {
  const rewritten = crumbs.map(c => ({
    ...c,
    label: c.label === 'Market Insights' ? (pageLabel ?? 'Analytics') : c.label,
    url: rewriteUrl(c.url, basePath),
  }));
  return rewritten;
}

const PageShellHeader: React.FC<PageShellHeaderProps> = ({ scope, basePath, pageLabel }) => {
  const kpis = scope.getKpis();
  const score = scope.getConditionScore();
  const label = scope.getMarketLabel();
  const breadcrumbs = rewriteBreadcrumbs(scope.getBreadcrumbs(), basePath, pageLabel);
  ...
  return (
    <div className="bg-[#0C1C2E] py-3 print:hidden">
