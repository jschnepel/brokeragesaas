...
-- Target: RDS (ARMLS mirror)
-- Purpose: Pre-aggregated rollups for public-facing market analytics
-- Refresh: Weekly minimum (cron or manual REFRESH MATERIALIZED VIEW)
-- Note: Uses ROUND() and wide numeric types to handle edge-case data (e.g. close/list > 1.0)

BEGIN;

-- ============================================
-- MV 1: MARKET PULSE — Monthly aggregates per geo tier
-- Powers: KPI cards, price trend charts, absorption rates
-- ============================================
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_market_pulse_monthly AS
SELECT
  date_trunc('month', COALESCE(close_date, listing_contract_date, original_entry_timestamp::date)) AS month,
  city, postal_code, subdivision_name, standard_status,
...
GROUP BY month, city, postal_code, subdivision_name, standard_status;

-- ============================================
-- MV 2: PRICE BAND DISTRIBUTION
-- Powers: Price band histograms by geo
-- ============================================
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_price_band_distribution AS
SELECT city, postal_code, subdivision_name, standard_status,
...
GROUP BY city, postal_code, subdivision_name, standard_status, price_band, band_order;

-- ============================================
-- MV 3: SUPPLY & DEMAND — New listings vs closings per month
-- Powers: Supply/demand dual-line charts, absorption rates
-- ============================================
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_supply_demand_monthly AS
SELECT month, city, postal_code, subdivision_name,
...
GROUP BY month, city, postal_code, subdivision_name;

-- ============================================
-- MV 4: INVENTORY AGE — Active listings by DOM bands
-- Powers: Inventory age donuts, stale inventory detection
-- ============================================
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_inventory_age AS
SELECT city, postal_code, subdivision_name,
...
GROUP BY city, postal_code, subdivision_name, dom_band, band_order;

-- ============================================
-- MV 5: COMMUNITY SCORECARD — One row per subdivision
-- Powers: Community comparison, rankings, radar charts
-- Note: close_to_list filtered < 5 to exclude data errors (e.g. land relist at higher price)
-- ============================================
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_community_scorecard AS
SELECT city, postal_code, subdivision_name,
...
GROUP BY city, postal_code, subdivision_name HAVING COUNT(*) >= 3;

-- ============================================
-- MV 6: HEATMAP POINTS — Lat/lng with metrics for H3 aggregation
-- Powers: H3 hexagonal heatmap on /market page
-- ============================================
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_heatmap_points AS
SELECT listing_key, latitude, longitude, subdivision_name, city, postal_code, list_price, close_price,
  CASE WHEN living_area > 0 THEN (COALESCE(close_price, list_price) / living_area)::numeric(14,2) ELSE NULL END AS price_per_sqft,
  days_on_market,
  CASE WHEN close_price > 0 AND list_price > 0 THEN (close_price / list_price * 100)::numeric(8,2) ELSE NULL END AS sale_to_list_pct,
  standard_status
