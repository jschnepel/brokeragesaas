      ...
      <section className="relative w-full flex items-end" style={{ height: '65vh', minHeight: '400px' }}>
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div className="hidden md:block w-full h-full cursor-pointer" data-hero-photo>
            <img src={gallery[0]} alt={streetAddress} className="w-full h-full object-cover" />
          </div>
          <div className="md:hidden w-full h-full overflow-x-auto scrollbar-hide snap-x snap-mandatory flex" data-hero-photo>
            {gallery.map((url, i) => (
              <div key={i} className="w-full h-full flex-shrink-0 snap-center">
                <img src={url} alt={`${streetAddress} photo ${i + 1}`} className="w-full h-full object-cover" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="hidden md:grid grid-cols-4 gap-0.5">
          {gallery.slice(1, 4).map((url, i) => (
            <div key={i} className="relative aspect-[16/9] overflow-hidden group/thumb cursor-pointer">
              <img src={url} alt={`${streetAddress} photo ${i + 2}`} className="w-full h-full object-cover group-hover/thumb:scale-105 transition-transform duration-500" data-photo-index={i + 1} />
            </div>
          ))}
          <div className="relative aspect-[16/9] overflow-hidden cursor-pointer" data-photo-index={4}>
            {gallery.length > 4 ? (
              <>
                <img src={gallery[4]} alt={`${streetAddress} photo 5`} className="w-full h-full object-cover" />
